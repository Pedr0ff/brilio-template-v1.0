...
(function ($) {
	...
	$( document ).ready(function() {
		...
		window.addEventListener('scroll', function () {
			...
			// Vérifie que window.myNavbarRef et window.myNavbarRef.current existent
			if (window.myNavbarRef && window.myNavbarRef.current) {
				const navbar = window.myNavbarRef.current;
				...
				// position = scroll; // Cette ligne n'est plus strictement nécessaire,
				// car on utilise directement window.scrollY.  Tu peux la garder
				// si tu utilises 'position' ailleurs dans ton code, mais
				// assure-toi qu'elle est correctement initialisée (let position = window.scrollY; au début du fichier).
			}
		});
		...
		if (filterItems.length) {
			document.querySelectorAll('.explore-area').forEach((exploreArea, index) => {
				const count = index + 1;
				...
				// Initialize Shuffle
				const Filter = new Shuffle(document.querySelector(`.filter-items-${count}`), {
					itemSelector: `.filter-item-${count}`,
					buffer: 1,
				});

				// Add event listeners
				exploreArea.querySelectorAll(`.filter-btn-${count}`).forEach(btn => {
